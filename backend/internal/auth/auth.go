...
func (s *Service) Login(ctx context.Context, req LoginRequest) (LoginResponse, error) {
	...
	if err != nil {
		return LoginResponse{}, web.Unauthorized("username or password is incorrect")
	}
	if !user.Active {
		return LoginResponse{}, web.Forbidden("account is inactive")
	}
	...
	if err != nil {
		return LoginResponse{}, web.Internal(fmt.Errorf("sign jwt: %w", err))
	}
...
func (s *Service) Parse(raw string) (Claims, error) {
	...
	if err != nil || !parsed.Valid {
		return Claims{}, web.Unauthorized("session token is invalid or expired")
	}
	claims, ok := parsed.Claims.(*Claims)
	if !ok || claims.UserID == 0 || claims.Role == "" {
		return Claims{}, web.Unauthorized("session token is missing claims")
	}
	return *claims, nil
}
...
func (s *Service) CurrentPrincipal(ctx context.Context, id uint) (User, error) {
	...
	if err != nil {
		return User{}, web.Unauthorized("session account is unavailable")
	}
	if !user.Active {
		return User{}, web.Unauthorized("session account is inactive")
	}
	return user, nil
}
...
func SeedUsers(db *gorm.DB) error {
	seeds := []struct{ Username, DisplayName, Role, Password string }{
		{"planner", "Capacity Planner", RolePlanner, "planner123"},
		{"reviewer", "Thermal Reviewer", RoleReviewer, "reviewer123"},
		{"admin", "Platform Administrator", RoleAdmin, "admin123"},
	}
