...
func TestCurrentPrincipalUsesCurrentAccountState(t *testing.T) {
	...
	if err := db.Model(&user).Updates(map[string]any{"role": RoleReviewer}).Error; err != nil {
	...
	if principal.Role != RoleReviewer {
		t.Fatalf("role = %q, want %q", principal.Role, RoleReviewer)
	}
