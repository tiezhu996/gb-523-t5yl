...
评估时方案会保存完整输入快照：全部热区、全部机柜、方案所选设备负载。之后规划员若改动机柜（功率/气流/U 位/位置/状态）、热区（冷量/温度/邻接/状态）或所选负载（功率/热量/气流/U 位/冗余组/偏好热区/状态），方案列表、规划详情和审计页会对该方案显示 **“Input changed（输入已变化）”** 警示，逐条列出变化的实体类型、编码、字段及前后取值。新增/删除的机柜和热区、删除的所选负载同样会被标出。

- 复核员对过期方案点击批准时，按钮禁用；即使绕过前端，服务端也会在批准事务前按评估快照与当前输入再核对一次，返回 `409 SCENARIO_INPUTS_CHANGED`，提示先退回 draft 重新评估。
- 重新评估后快照刷新，警示自动消失；旧方案不会被删除，仍可在“版本比较”中与当前结果对照，比较页两侧都会标注各自是否已过期。
- 邻接权重按解码后的语义比较，JSON 键顺序变化不会误报。
...
- 批准返回 `SCENARIO_INPUTS_CHANGED`（409）：评估后机柜、热区或所选负载被改过，页面会列出具体变化项；退回 draft 重新评估后警示消失，再批准即可。
