...
@Component({
  ...
  imports: [DatePipe, DecimalPipe, MatButtonModule, MatFormFieldModule, MatSelectModule, CapacityMeterComponent,
    ConstraintBadgeComponent, InputChangesBannerComponent, VersionComparePanelComponent,
    LucideAngularModule],
      ...
      <section class="review-queue">
        ...
        <div class="queue-items">
          @for (scenario of pending(); track scenario.id) {
            <article [class.stale]="scenario.inputs_changed_since_evaluation">
              <div class="queue-head"><span class="status pending_review">pending review</span><h2>{{ scenario.name }}</h2><p>v{{ scenario.version }} / {{ scenario.algorithm_version }} / {{ scenario.assignments.length }} placements</p></div>
              <div class="queue-score"><small>Score</small><strong>{{ scenario.score | number:'1.0-1' }}</strong></div>
              @if (scenario.inputs_changed_since_evaluation) {
                <app-input-changes-banner class="queue-details" [changes]="scenario.input_changes" />
                <app-constraint-badge severity="critical" label="Inputs changed" />
              } @else {
              ...
              <div class="queue-actions">
                <button mat-flat-button color="primary" [disabled]="scenario.has_critical_violation || scenario.inputs_changed_since_evaluation" (click)="transition(scenario, 'approved')"><lucide-icon name="check-circle-2" [size]="16" /> Approve</button>
                @if (scenario.inputs_changed_since_evaluation) {<button mat-stroked-button (click)="transition(scenario, 'draft')"><lucide-icon name="rotate-ccw" [size]="15" /> Return for re-evaluation</button>}
              </div>
            </article>
...
export class AuditPage {
  ...
  transition(scenario: LayoutScenario, target: ScenarioStatus): void {
    const reason = target === 'approved' ? 'Approved after thermal review'
      : target === 'draft' ? 'Inputs changed since evaluation; re-evaluate before review'
