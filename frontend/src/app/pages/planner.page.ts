...
import { InputChangesBannerComponent } from '../components/common/input-changes-banner.component';
...
@Component({
  ...
  imports: [DecimalPipe, ReactiveFormsModule, MatButtonModule, MatCheckboxModule, MatFormFieldModule, MatInputModule, MatSelectModule,
    ConstraintBadgeComponent, InputChangesBannerComponent, LucideAngularModule],
    ...
    <div class="page planner-page">
      ...
      <section class="control-bar">
        ...
        @if (selected(); as active) {
          ...
          @if (active.scenario_status === 'pending_review' && canApprove()) {<button mat-flat-button color="primary" (click)="transition('approved')" [disabled]="active.has_critical_violation || active.inputs_changed_since_evaluation"><lucide-icon name="check-circle-2" [size]="16" /> Approve</button>@if (active.inputs_changed_since_evaluation) {<button mat-stroked-button (click)="transition('draft')"><lucide-icon name="rotate-ccw" [size]="15" /> Return for re-evaluation</button>}}
        }
      </section>

      @if (selected(); as active) {
        @if (active.inputs_changed_since_evaluation) {
          <app-input-changes-banner class="stale-strip" [changes]="active.input_changes" [hint]="active.scenario_status === 'draft' ? 'Send the draft through evaluation again to refresh this result.' : 'Return the scenario to draft and re-evaluate before approving.'" />
        }
      }
...
export class PlannerPage {
  ...
  transition(target: ScenarioStatus): void { const scenario = this.selected(); if (!scenario) return; const reason = target === 'draft' ? 'Inputs changed since evaluation; re-evaluate before review' : 'Reviewed in planning workbench'; this.scenarioApi.transition(scenario.id, scenario.version, target, reason).subscribe((result) => { this.store.select(result); this.scenarios.update((items) => items.map((item) => item.id === result.id ? result : item)); this.snack.open(`Scenario ${target.replace('_', ' ')}`, undefined, {duration: 2200}); }); }
