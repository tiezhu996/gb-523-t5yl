...
@Component({
  ...
  imports: [DecimalPipe, ReactiveFormsModule, MatButtonModule, MatCheckboxModule, MatFormFieldModule, MatInputModule, MatSelectModule,
    ConstraintBadgeComponent, InputFreshnessBannerComponent, LucideAngularModule],
    ...
    <div class="page planner-page">
      ...
      <section class="control-bar">
        ...
        @if (selected(); as active) {
          ...
          @if (isStale(active)) {<span class="status critical"><lucide-icon name="alert-triangle" [size]="12" /> inputs changed</span>}
          ...
          @if (active.scenario_status === 'pending_review' && canApprove()) {<button mat-flat-button color="primary" (click)="transition('approved')" [disabled]="active.has_critical_violation || isStale(active)"><lucide-icon name="check-circle-2" [size]="16" /> Approve</button>}
          @if (active.scenario_status === 'pending_review' && isStale(active) && canApprove()) {<button mat-stroked-button color="warn" (click)="transition('draft')"><lucide-icon name="rotate-ccw" [size]="16" /> Return to draft</button>}
        }
      </section>

      @if (selected(); as active) {
        <app-input-freshness-banner [freshness]="active.input_freshness" />
      }
